...
### Header-sync fix (TODO row 32) reverted -- a real regression found on the actual CI run, not just theory

The row-32 fix (deleting both inherited "Qtum" timestamp-based anti-reorg checks, keeping the height-based
`nMaxReorganizationDepth`/`CheckSyncCheckpoint` checks) was applied with sign-off, built cleanly, and passed a
live two-node regtest sanity check (isolate a node, mine 500 blocks on the other, reconnect, confirm a clean
sync). That sanity check was not enough: the fix's actual effect on the real functional suite, on the next
genuine `win64-native` CI run, was worse than doing nothing.

`feature_bip68_sequence.py` failed with the **exact same** `AssertionError: Block sync timed out after 2400s`
at the exact same point (`activateCSV()` -> `sync_blocks()`) as before the fix -- meaning the timestamp check
was never actually the (sole) cause of this specific test's hang, despite the earlier investigation's
confident-sounding root-cause writeup. Worse, three tests that had never failed in any previous round started
failing: `mining_basic.py`, `rpc_getchaintips.py`, and both variants of `wallet_conflicts.py`.
`mining_basic.py`'s actual error, `bad-fork-prior-to-synch-checkpoint (-25)`, comes from the height-based
`CheckSyncCheckpoint()` check -- the one this fix deliberately *kept* as "the correct one." The working theory:
removing the timestamp check let headers reach `ContextualCheckBlockHeader()`'s height check in situations
that previously never got that far (the timestamp check, bug and all, was apparently rejecting -- or
otherwise diverting -- some of these cases earlier in the pipeline), exposing a second, separate bug in the
"correct" check rather than fixing the first one.

Reverted both files back to their exact pre-fix state (`git checkout` from the commit before the fix,
confirmed via a clean VPS rebuild) rather than attempt a second live guess against consensus-adjacent P2P
code. This is a genuine lesson, not just an inconvenience: a narrow, hand-picked regtest sanity check
confirmed the fix didn't break the *one specific scenario* it was designed around, but said nothing about the
rest of the functional suite, and the rest of the suite is exactly where the real damage showed up. The
"root cause, fix, verify with a small regtest check" pattern that worked well for wallet-layer bugs earlier
this session is not sufficient by itself for P2P/consensus code -- the real functional suite, or at minimum a
targeted repro of `feature_bip68_sequence.py`'s exact scenario, needs to be the verification step before
trusting a fix like this again.

The very next `win64-native` run (confirming the revert itself, commit `32ff762`) is worth one more note:
`feature_bip68_sequence.py` still failed, but with a *different* symptom than every prior run -- `assert
(all([len(x.getpeerinfo()) for x in rpc_connections]))` at `sync_blocks()`, meaning one node had zero peers at
that check, not the ~2400s "Block sync timed out" this test has shown on every other run (this one failed in
617s instead). That's either a one-off CI network flake unrelated to any of this investigation, or a real
...
| 32 | **Fix applied, then reverted after a real `win64-native` run showed it made things worse, not better.** `src/validation.cpp`'s and `src/net_processing.cpp`'s inherited "Qtum" sync-checkpoint timestamp checks fire on ordinary header-first sync, not just genuine reorgs. The proposed fix (delete both timestamp-check copies, keep the height-based `nMaxReorganizationDepth`/`CheckSyncCheckpoint` checks) was applied with sign-off and passed this session's own two-node regtest sanity check, but on the actual `win64-native` CI run: (a) `feature_bip68_sequence.py` **still failed with the identical 2400s timeout** -- the fix had zero effect on the bug it was meant to fix, meaning the root-cause diagnosis was wrong or incomplete; and (b) three tests that had never failed before (`mining_basic.py`, `rpc_getchaintips.py`, `wallet_conflicts.py` both variants) started failing, `mining_basic.py` with `bad-fork-prior-to-synch-checkpoint (-25)` -- the height-based check I deliberately left in place as "the correct one." Removing the timestamp check apparently let headers reach that height check in situations that previously never got that far, exposing a second, previously-masked bug rather than fixing the first one. Reverted both files back to their pre-fix state (confirmed via a clean VPS rebuild, and via the next real `win64-native` run: the three new regressions are gone, back to exactly the previously-documented set of failures) rather than attempt a second guess live in production code -- see the "Header-sync fix reverted" section above for the full writeup, including a note that the very next run's `feature_bip68_sequence.py` failure had a different symptom (a peer-count assertion, not the usual 2400s timeout) worth a second look. This needs real, careful investigation (ideally ONE isolated repro of `feature_bip68_sequence.py`'s exact failure, not a guess against the described mechanism) before touching this code again | Phase 2 |
