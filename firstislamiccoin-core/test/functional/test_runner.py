...
BASE_SCRIPTS = [
    ...
    'feature_taproot.py',
    'feature_block.py',
    # FirstIslamicCoin: genesis premine, fixed staking reward, PoS reorg
    'feature_fic_fixed_reward.py',
    'feature_fic_genesis_premine.py',
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if firstislamiccoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "firstislamiccoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a firstislamiccoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
