...
def create_transactions(node, address, amt, fees):
    ...
    for fee in fees:
        ...
        # FirstIslamicCoin: createrawtransaction takes no 'replaceable' argument (no opt-in RBF)
...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            # Tests legacy watchonly behavior which is not present (and does not need to be tested) in descriptor wallets
            assert_equal(self.nodes[0].getbalances()['mine']['trusted'], POW_SUBSIDY)
            assert_equal(self.nodes[0].getwalletinfo()['balance'], POW_SUBSIDY)
            assert_equal(self.nodes[1].getbalances()['mine']['trusted'], POW_SUBSIDY)

            assert_equal(self.nodes[0].getbalances()['watchonly']['immature'], COINBASE_MATURITY * POW_SUBSIDY)
            assert 'watchonly' not in self.nodes[1].getbalances()
        ...
        assert_equal(self.nodes[0].getbalance("*"), POW_SUBSIDY)
        assert_equal(self.nodes[0].getbalance("*", 1), POW_SUBSIDY)
        assert_equal(self.nodes[0].getbalance(minconf=1), POW_SUBSIDY)
        if not self.options.descriptors:
            assert_equal(self.nodes[0].getbalance(minconf=0, include_watchonly=True), 2 * POW_SUBSIDY)
            assert_equal(self.nodes[0].getbalance("*", 1, True), 2 * POW_SUBSIDY)
        else:
            assert_equal(self.nodes[0].getbalance(minconf=0, include_watchonly=True), POW_SUBSIDY)
            assert_equal(self.nodes[0].getbalance("*", 1, True), POW_SUBSIDY)
        assert_equal(self.nodes[1].getbalance(minconf=0, include_watchonly=True), POW_SUBSIDY)

        # FirstIslamicCoin: each block pays POW_SUBSIDY (S) instead of 50. Node 1's send is
        # S + 10 (upstream: 60 == 50 + 10) so it still needs node 0's unconfirmed output;
        # node 1's figures are then unchanged and node 0's are shifted by S - 50.
        # Send 40 BTC from 0 to 1 and S + 10 BTC from 1 to 0.
        txs = create_transactions(self.nodes[0], self.nodes[1].getnewaddress(), 40, [Decimal('0.01')])
        self.nodes[0].sendrawtransaction(txs[0]['hex'])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])  # sending on both nodes is faster than waiting for propagation

        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), POW_SUBSIDY + 10, [Decimal('0.01'), Decimal('0.02')])
        ...
        def test_balances(*, fee_node_1=0):
            # getbalances
            # FirstIslamicCoin: getbalances also reports 'stake' (immature coinstake outputs)
            ...
            assert_equal(self.nodes[0].getbalance(), POW_SUBSIDY - Decimal('40.01'))  # change from node 0's send
            assert_equal(self.nodes[1].getbalance(), Decimal('0'))  # node 1's send had an unsafe input
            # Same with minconf=0
            assert_equal(self.nodes[0].getbalance(minconf=0), POW_SUBSIDY - Decimal('40.01'))
            ...
            assert_equal(self.nodes[0].getunconfirmedbalance(), POW_SUBSIDY + 10)  # output of node 1's spend
            ...
            assert_equal(self.nodes[0].getwalletinfo()["unconfirmed_balance"], POW_SUBSIDY + 10)
        ...
        # (upstream: 99 of ~99.86; FirstIslamicCoin balances are ~2 * POW_SUBSIDY)
        big_amount = int(2 * POW_SUBSIDY - 1)
        for _ in range(3):
            # Set amount high enough such that all coins are spent by each tx
            txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), big_amount)
        ...
        tx_replace = tx_orig.replace(
            struct.pack("<q", big_amount * 10**8).hex(),
            struct.pack("<q", (big_amount - 1) * 10**8).hex(),
        )
