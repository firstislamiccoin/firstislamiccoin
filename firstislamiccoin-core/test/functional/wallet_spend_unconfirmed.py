...
class UnconfirmedInputTest(BitcoinTestFramework):
    ...
    def test_chain_of_unconfirmed_low(self):
        ...
        grandparent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.8, fee_rate=100)
        gp_tx = wallet.gettransaction(txid=grandparent_txid, verbose=True)

        self.assert_undershoots_target(gp_tx)

        parent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.5, fee_rate=100)
    ...
    def test_two_low_feerate_unconfirmed_parents(self):
        ...
        parent_one_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.5, fee_rate=100)
        p_one_tx = wallet.gettransaction(txid=parent_one_txid, verbose=True)
        self.assert_undershoots_target(p_one_tx)

        parent_two_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.5, fee_rate=100)
    ...
    def test_mixed_feerate_unconfirmed_parents(self):
        ...
        parent_low_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.5, fee_rate=100)
    ...
    def test_chain_of_high_low_below_target_feerate(self):
        ...
        grandparent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=1.8, fee_rate=150)
        gp_tx = wallet.gettransaction(txid=grandparent_txid, verbose=True)
    ...
    def test_sibling_tx_bumps_parent(self):
        ...
        # …because the sibling bumps the parent's ancestry feerate above target on its own
    ...
    def test_external_input_unconfirmed_low(self):
        ...
        parent_txid = wallet.sendtoaddress(address=external_address, amount=1, fee_rate=100)
        parent_tx = wallet.gettransaction(txid=parent_txid, verbose=True)
    ...
    def run_test(self):
        ...
        # FirstIslamicCoin: sendtoaddress/send hard-reject any fee_rate below
        # the fixed 100 sat/vB minimum (GetMinFee) rather than silently
        # raising it, so every "low" parent below explicitly requests exactly
        # the floor (100) instead of upstream's arbitrary 1/2/5. Keep the
        # target well above that so "low" still means something.
